...
\title{OSTEP missing chapter}
\author{ben.reed }
\date{December 2021}

\begin{document}

\textbf{Student:} Hi Professor, I am taking your operating systems class this semester.

\vspace{\baselineskip}

\textbf{Professor:} Welcome! You can just call me Ben.  This is my favorite class to teach. Operating systems are where our code runs. Understanding that environment can allow you to do some magical things.

\vspace{\baselineskip}

\textbf{Student:} What is this chapter? I thought you said the book is amazing. Why do we need more?

\vspace{\baselineskip}

\textbf{Professor:} The book is amazing! (And free which is also amazing!) I noticed that the book was missing a description of how operation systems start and a high level view of the architecture. It also assumes that students have a good idea of what kernel-mode and user-mode is.

\vspace{\baselineskip}

\textbf{Student:} So, what is an operating system?

\vspace{\baselineskip}

\textbf{Professor:} Oh, that is a great question! Different people have different definitions of what an operating system is. Let's come back to that question after you have read this chapter.

\vspace{\baselineskip}

\textbf{Student:} Uh, okay... Well, what will I learn in this class.

\vspace{\baselineskip}

\textbf{Professor:} Another great question! I guess that is really up to you. I'll introduce you to some ideas in lecture. We are using a great book. And, I have some great programming assignments to make sure that you have mastered the concepts we covered. Oh, I guess the exams will also help make sure you've mastered the concepts. In the end though, what you learn depends on you.

\vspace{\baselineskip}

\textbf{Student:} Hmm, I see. Okay. Well, one last question: when will Professor Arpaci-Dusseau be back?

\vspace{\baselineskip}

\textbf{Professor:} Next chapter.
...
We call this initial startup process \emph{boot up} and say that the computer is \emph{booting}.
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/boot-timeline.png}
    \caption{System startup timeline.}
    \label{fig:boot}
\end{figure}

Figure \ref{fig:boot} shows a timeline of the different programs involved in booting an operating system.
...
It seems natural to start executing code at address $\mathtt{0x00000000}$, but that is not the normal initial jump address for popular CPUs.
The x86\_64 CPUs, found in most PCs, start executing at the address $\mathtt{0xFFFFFFF0}$~\cite{intel:asdm}.
ARM based processors, found in most phones and new Apple products, boot from the address $\mathtt{0x00000000}$ or $\mathtt{0xFFFF0000}$ depending on a hardware configuration \cite{arm}.
As with x86\_64 CPU systems this address is usually found in ROM.
However, we will see a variation of this in section \ref{sec:pi}.
Even Intel based Macs have a slightly different boot process \cite{macboot}.

\subsection{BIOS and UEFI}
Usually, there is flash memory, also referred to as \emph{firmware} at that address containing the initial program.
...
To get this flexibility, they use a \emph{layer of indirection}.
...
Historically, this first program that is run is called \emph{BIOS} (Basic Input/Output System) in PCs, but newer computers use \emph{UEFI} (Unified Extensible Firmware Interface).
...
\begin{figure}
\begin{boxed}
\subsection*{The two solutions to all computer science problems}
...
\setlength\itemsep{0em}
    \item Caching: storing results to reuse in the future to improve performance.
    \item Level of indirection: adding a layer of access to an object for flexibility and efficiency.
\end{itemize}

An often quoted corollary to the level of indirection is ``except for the problem of too many layers of indirection'', which can be solved by caching. Note that these techniques also give rise to the two hard problems in computer science:
...
While the indirection provided by UEFI allows the manufacturer of the computer to provide some flexibility at startup, operating system vendors would also like flexibilty as well.
Sometimes, operating systems have ``safe mode''s that allow the system to boot with just basic functionality, for example.
For this reason, UEFI usually loads a \emph{bootloader} that knows how to load the operating system.
...
\begin{figure}
    \centering
    \includegraphics[height=.5\textheight]{images/raspberry-pi-boot-process.jpg}
    \caption{Raspberry Pi boot process. Image from \cite{piboot} which has more details of the process.}
    \label{fig:piboot}
\end{figure}
...
Figure \ref{fig:piboot} shows a flow similar to the one that we just discussed except that the first processor involved is the GPU (Graphical Processing Unit) rather than the CPU, which is an ARM processor on the Pi.
...
\section{The kernel}
The program usually loaded by a bootloader is the operating system \emph{kernel}.
...
First, the kernel does not go away\footnote{There are parts of UEFI, BIOS, and other management systems that do stick around but they are outside the scope of this discussion and can be ignored for this discussion.}.
...
They run in a lesser privileged mode called \emph{user-mode}.
In user-mode, running programs (called \emph{processes}) are not able to access some of the more powerful instructions and registers, but with those exceptions when they run, they have full control of the CPU.
...
Unix-like kernels start a first process called \texttt{init}.
Each process in the system is assigned a number, and \texttt{init} is appropriately assigned the number $1$\footnote{Hmm, is it really appropriate to start at 1? Many programmers would say that the first number is 0 ;) There is that off-by-one again!}.
This process is in charge of starting up the rest of the programs that the system needs.

Sometimes \texttt{init} is as simple as a script that starts up a list of other programs and never terminates, but usually \texttt{init} is a more sophisticated program that manages program dependencies.
Systemd is currently a commonly used \texttt{init} program in Linux systems.
...
The \texttt{init} process will continue running and monitoring the programs it has started.
Often times, the \texttt{init} process will be expected to restart programs if they fail.
If the \texttt{init} process ever finishes, the system will crash.

\subsection{System calls}

\begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{images/monolith.png}
    \caption{An example of \texttt{myapp} opening a file by invoking a system call.}
    \label{fig:monolith}
\end{figure}
...
Fortunately, the kernel, which runs in \emph{kernel-mode}, can do all those things.

A process asks the kernel to do things on its behalf using \emph{system calls}.
Software interrupts or special system call instructions\footnote{Appropriately named SYSCALL on x86 and SVC on ARM.} allow a process to transfer control to the kernel and switch into kernel-mode.
...
Figure \ref{fig:monolith} shows an example of an application, \texttt{myapp} invoking a system call to open a file.
...
Returning results back to \texttt{myapp} requires data to be copied from kernel memory into the application's memory.
...
For example, on Linux and OSX, we list the files in a directory using \texttt{ls}, but on Windows we use \texttt{dir}.
There are also commands like \texttt{find} to search files in the file system and \texttt{grep} to search the contents of files.
The command \texttt{xterm} creates a terminal, and the window manager commands start the graphical environments that we work in.
...
For example, \texttt{zsh} the default shell on OSX was written by a student, Paul Falstad, at Princeton in 1990.
Hisham Muhammad developed \texttt{htop}, the popular replacement for \texttt{top}, just before starting his Master's at Pontifícia Universidade Católica do Rio de Janeiro.
...
It turns out that it is just a matter of where the program resides and the value of the \texttt{PATH} environment variable.
We usually think of a command as something that we can run by just typing its name.
For example, we type \texttt{ls} to list the files in a directory.
But, we can also type \texttt{/usr/bin/ls}.

The locations that the operating system looks for commands is governed by the \texttt{PATH} environment variable.
You can see the list of directories that it looks for commands by typing \texttt{echo "\$PATH"}.
...
We write the classic one-liner in \texttt{hello.c} and compile it to \texttt{hello}.
If we copy it into \texttt{/usr/bin}, we can type \texttt{hello} anytime we want for a pleasant greeting.

In practice, you shouldn't put personal commands into \texttt{/usr/bin}.
Usually, we create a \texttt{bin} or \texttt{.local/bin} in our home directory to store our personal commands.
Often, your operating system distribution will have a default personal \texttt{bin} directory.
Run \texttt{echo "\$PATH"} and look through the directories to see if you already have something set up.
...
We call these kernels \emph{monolithic kernels}.

\emph{Micro kernels} provide an alternative way of implementing operating systems kernels.
...
The kernel communicates with these services, and services communicate with each other, through Inter-Processes Communication {IPC} mechanisms.
...
\begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{images/micro.png}
    \caption{An example of \texttt{myapp} opening a file by invoking a system call in a microkernel.}
    \label{fig:micro}
\end{figure}

Figure \ref{fig:micro} shows \texttt{myapp} invoking a system call to open a file in a microkernel.
In a microkernel, the file system is running outside the kernel but the kernel mediates the communication between \texttt{myapp}.
The kernel ensures that \texttt{myapp} and the file system are isolated from each other, but it also needs to secure and efficiently provide communication between the two endpoints.
...
\begin{figure}
\begin{boxed}
\subsection*{Linus Torvalds and Professor Tanenbaum}
In 1992, there was an interesting debate (AKA flamewar)~\cite{linux-ast} on a USENET (message board) channel about the first version of Linux.
...
\section{Summary: What is an operating system?}
...
It is natural that users of ChromeOS would consider the Chrome browser to be part of the the operating system, Chrome browser developers are not generally considered operating system developers.
